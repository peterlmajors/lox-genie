...
def get_nfl_leagues_user(user_id: int, year: int = settings.NFL_YEAR):
    """
    ...
    with httpx.Client() as client:
        ...
        if response.status_code == 200:
            ...
            raise Exception(f"Failed to fetch leagues for user {user_id}: {response.status_code}")
        
def get_team_performances(league_id: int, week: int = get_current_state_nfl()['week']):
    """
    ...
    with httpx.Client() as client:
        ...
        if response.status_code == 200:
            ...
            raise Exception(f"Failed to fetch matchups for week {week}: {response.status_code}")
