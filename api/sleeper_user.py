...
from functions.sleeper.utils import get_nfl_leagues_user
...
def get_nfl_leagues_user_metadata(user_id: int, season: int = settings.NFL_YEAR) -> list:
    ...
    if leagues:
        ...
        for league in leagues:
            ...
            keys_to_keep = ["league_id", "draft_id", "name", "status", "season_type", "total_rosters", "roster_positions", "scoring_settings", "metadata", "previous_league_id"]
            ...
            if "roster_positions" in filtered_league:
                filtered_league["roster_positions"] = dict(Counter(filtered_league["roster_positions"]))

            # Filter the scoring settings
            scoring_settings = filtered_league.get("scoring_settings", {})
            filtered_league["offense_scoring"] = {k: v for k, v in scoring_settings.items() if k in settings.OFFENSE_STATS}
            filtered_league["defense_scoring"] = {k: v for k, v in scoring_settings.items() if k in settings.DEFENSE_STATS}
            filtered_league["kicker_scoring"] = {k: v for k, v in scoring_settings.items() if k in settings.KICKER_STATS}
            filtered_league.pop("scoring_settings", None)  # Remove the original scoring settings
...
@router.get("/users/{user_id}/leagues/{league_id}/waiver_budget")
...
@router.get("/users/{user_id}/leagues/{league_id}/win_percentage")
