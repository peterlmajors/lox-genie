...
from functions.sleeper.api import get_league_users, get_all_draft_picks, get_all_league_drafts
...
def get_all_draft_picks_metadata(league_id: int, draft_id: int) -> list[dict]:
    ...
    draft = next(draft for draft in get_all_league_drafts(settings.SLEEPER_IFL_24) if draft["draft_id"] == str(draft_id))
...
async def get_league_picks(league_id: str) -> dict:
    try:
        league_picks = []
        ...
        raise HTTPException(status_code=502, detail=f"Failed to fetch league picks: {e}")