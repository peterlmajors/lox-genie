import pandas as pd
from services.genie.api.services.sleeper.api import get_league_users
    ...
    try:
        ...
        if roster.get(dict_name):
            if dict_name == "user_id":
                ...
                if not user:
                    raise Exception(
                        f"match_id_metadata(): No match found for user_id {roster['user_id']}"
                    )
                user.pop("user_id")
                return user
            else:
                for id in roster[dict_name]:
                    try:
                        ...
                        print(
                            f"match_id_metadata(): Error matching {dict_name} id {id}: {e}"
                        )
                        continue
        return resp
    except Exception as e:
        print(f"match_id_metadata(): General error: {e}")
        return None


def roster_dict_to_df(roster: dict):
